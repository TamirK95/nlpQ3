...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # step 1: Randomly truncate the document to a length no less than 4 characters,and no more than int(self.block_size*7/8) characters.
        maxChars = int(self.block_size*7/8)
        minChars = 4
        doc = doc[:random.randint(minChars, maxChars)]

        # step 2: Now, breaking the document into three substrings: [prefix] [masked_content] [suffix]
        mean = int(len(doc)/4)
        start = round(mean*0.5)
        end = round(mean*1.5)
        maskLen = random.randint(start, end)
        prefixLen = random.randint(0, len(doc)-maskLen)
        suffixLen = len(doc) - maskLen - prefixLen

        prefix = doc[:prefixLen]
        masked = doc[prefixLen:prefixLen+maskLen]
        suffix = doc[prefixLen+maskLen:]

        # step 3: Rearrange these substrings as described
        ...
        inputString = masked_string[:-1]
        outputString = masked_string[1:]
        
        # step 5: encode the resulting input and output strings as Long tensors, return the resulting data point
        inputArr = [self.stoi[s] for s in inputString]
        inputTensor = torch.tensor(inputArr, dtype=torch.long)
        outputArr = [self.stoi[s] for s in outputString]
        outputTensor = torch.tensor(outputArr, dtype=torch.long)

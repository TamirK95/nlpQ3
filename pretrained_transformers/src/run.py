...
if __name__ == '__main__':
    ...
    if args.function == 'pretrain':
        ...
        pretrain_eval_dataset = None
        myTrainer = trainer.Trainer(model, pretrain_dataset, pretrain_eval_dataset, tconf)
        myTrainer.train()
        ...
        finetune_test_dataset = dataset.NameDataset(pretrain_dataset, finetune_corpus)
        finetune_eval_dataset = None

        if args.reading_params_path is None: # finetuning without a pretrained model [c]
            ...
            myTrainer = trainer.Trainer(model, finetune_test_dataset, finetune_eval_dataset, tconf)
            myTrainer.train()

            # Save the resulting model in args.writing_params_path
            myTrainer.save_checkpoint()
        else: # finetuning with a pretrained model
            # need to load thesegiven  parameters into the model
